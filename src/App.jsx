...
// ── View components (canonical source — NO duplicates in App.jsx) ──
import WaterfallView, { ExitAnalysisPanel, IncentivesImpact, HelpLink, EducationalModal } from "./components/views/WaterfallView";
import ResultsView, { SelfResultsView, BankResultsView, IncomeFundResultsView, FieldGroup, FL, Inp, Drp } from "./components/views/ResultsView";
import FinancingView from "./components/views/FinancingView";
import ProjectSetupWizard from "./components/shared/ProjectSetupWizard";
...
// ═══════════════════════════════════════════════════════════════
// VIEW COMPONENTS — Imported from canonical source files
// ═══════════════════════════════════════════════════════════════
// WaterfallView, ExitAnalysisPanel, IncentivesImpact → from ./components/views/WaterfallView
// ResultsView, SelfResultsView, BankResultsView, IncomeFundResultsView → from ./components/views/ResultsView
// FinancingView, FieldGroup, FL, Inp, Drp → from ./components/views/FinancingView + ResultsView
// ProjectSetupWizard → from ./components/shared/ProjectSetupWizard
//
// PREVIOUSLY: These were defined inline here (3,300+ lines of duplicate code).
// ARCHIVED TO: src/_deprecated_app_views/ (safe to delete after 2 weeks)
...
// QUICK SETUP WIZARD
// ═══════════════════════════════════════════════════════════════
// ProjectSetupWizard — imported from component files (removed local duplicate)
...
// HelpLink — imported from component files (removed local duplicate)
// EducationalModal — imported from component files (removed local duplicate)
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,marginBottom:16}}>
            <tbody>
              {[
                ...
                ...((100-(project.lpProfitSplitPct||100)) > 0 ? [[ar?"تقسيم الأرباح (بعد اللحاق)":"Profit Split (after catch-up)","Investor "+(project.lpProfitSplitPct||70)+"% / Developer "+(100-(project.lpProfitSplitPct||70))+"%"]] : []),
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,marginBottom:12}}>
              ...
              <tbody>
                {[
                  ...
                  [ar?"رسوم المطور":"Developer Fee",(project.developerFeePct||12)+"% "+(ar?"من CAPEX":"of CAPEX"),fmt(fw.feeDeveloper||0),ar?"خلال البناء":"During construction"],
...
function ChecksView({ checks, t, lang, onFix }) {
  ...
  const catLabels = {T0:ar?"T0: فحص المدخلات":"T0: Input Validation",T1:ar?"T1: محرك المشروع":"T1: Project Engine",T2:ar?"T2: التمويل":"T2: Financing",T3:ar?"T3: حافز حسن الأداء":"T3: Waterfall",T4:ar?"T4: الحوافز":"T4: Incentives",T5:ar?"T5: التكامل":"T5: Integration",General:"General"};
