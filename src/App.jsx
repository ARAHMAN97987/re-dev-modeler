...
• When comparing different financing scenarios`}>
        ...
        <AB id="exit" visible={true}>
          ...
          {notHold&&<>
            <div style={g2}>
              <div>
                <FL label={ar?"سنة التخارج":"Exit Year"} hint="0 = auto" tip="سنة بيع الأصل. عادة 5-10 سنوات بعد الاستقرار التشغيلي. 0 = تلقائي\nYear of asset sale. Usually 5-10 years after stabilization. 0 = auto" error={(cfg.exitYear||0)>0&&(cfg.exitYear||0)>(project?.horizon||50)?(ar?`سنة التخارج (${cfg.exitYear}) تتجاوز أفق المشروع (${project?.horizon})`:` Exit year (${cfg.exitYear}) exceeds horizon (${project?.horizon})`):null}><Inp type="number" value={cfg.exitYear} onChange={v=>upCfg({exitYear:v})} /></FL>
                    ...
                    <tr key={a.id||i} style={{background:bg}}>
                      ...
                      <td style={{...tdSt,...hd("opEbitda")}}>
                        <div style={{display:"flex",alignItems:"center",gap:4}}>
                          {isOp ? (
                            <>
                              <span style={{fontSize:11,color:"var(--text-secondary)",minWidth:55,textAlign:"right"}}>{fmtM(a.opEbitda||0)}</span>
                              {(isHotel||isMarina) && <button onClick={()=>setModal({type:isHotel?"hotel":"marina",idx:i})} style={{...btnSm,background:"#eef2ff",color:"#2563eb",fontSize:11,padding:"2px 6px",whiteSpace:"nowrap"}}>{isHotel?"P&L":"P&L"}</button>}
                            </>
                          ) : (
