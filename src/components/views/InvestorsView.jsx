...
// ── Header summary (module scope to preserve identity across renders) ──
function Summary({ ar, isMobile, totalCost, totalEquity, devEquity, invEquity, totalDebt }) {
  return (
    <div style={{
      ...cardStyle,
...
// ── Per-investor row (module scope — stable identity so input focus is preserved) ──
function InvestorRow({ inv, ar, isMobile, financing, totalEquity, updateInvestor, updateContribution, removeInvestor }) {
...
// ── Add-investor modal (module scope) ──
function AddModal({ ar, isMobile, presets, addInvestor, close }) {
  return (
    <div style={{
...
export default function InvestorsView({ project, updateProject, financing, lang }) {
  ...
  // Child components Summary / AddModal / InvestorRow are defined at module scope
  // above — this prevents React from recreating them every render (which would
  // unmount/remount each <input> and destroy keyboard focus mid-typing).
  ...
  return (
    <div dir={ar ? "rtl" : "ltr"} style={{ padding: isMobile ? "12px" : "16px 20px" }}>
      ...
      <Summary ar={ar} isMobile={isMobile} totalCost={totalCost} totalEquity={totalEquity} devEquity={devEquity} invEquity={invEquity} totalDebt={totalDebt} />
      ...
      {investors.map(inv => (
        <InvestorRow key={inv.id} inv={inv}
          ar={ar} isMobile={isMobile} financing={financing} totalEquity={totalEquity}
          updateInvestor={updateInvestor} updateContribution={updateContribution} removeInvestor={removeInvestor} />
      ))}
      ...
      {adding && <AddModal ar={ar} isMobile={isMobile} presets={presets} addInvestor={addInvestor} close={() => setAdding(false)} />}
    </div>
  );
}
